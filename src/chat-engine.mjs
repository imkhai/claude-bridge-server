...
export function buildAgentPrompt(agentId, userMessage, previousContext) {
 ...
 */
export async function spawnAgents(conv, routing, userMessage, files, pushUpdate) {
  try {
    ...
    if (method === 'chain') {
      await runChainAgents(conv, remainingAgents, userMessage, baseContext, taskIds, pushUpdate);
    } else if (method === 'parallel') {
      await runParallelAgents(conv, remainingAgents, userMessage, baseContext, taskIds, pushUpdate);
    } else {
      await runSingleAgentFlow(conv, remainingAgents[0], userMessage, baseContext, taskIds, pushUpdate);
    }
...
export async function runChainAgents(conv, agents, userMessage, baseContext, taskIds, pushUpdate) {
...
export async function runParallelAgents(conv, agents, userMessage, baseContext, taskIds, pushUpdate) {
...
export async function runSingleAgentFlow(conv, agentId, userMessage, baseContext, taskIds, pushUpdate) {
  const prompt = buildAgentPrompt(agentId, userMessage, baseContext);
